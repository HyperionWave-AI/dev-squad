...
func (h *ChatWebSocketHandler) streamAIResponse(ctx context.Context, conn *websocket.Conn, sessionID primitive.ObjectID, userMessage, companyID string) {
	...
	// Step 2.5: Get user's system prompt (or use default) and prepend it
	session, _ := h.chatService.GetSession(ctx, sessionID, companyID)
	userID := ""
	if session != nil {
		userID = session.UserID
	}
	systemPrompt := h.getSystemPromptOrDefault(ctx, userID, companyID)
...
// getSystemPromptOrDefault retrieves user's custom system prompt or returns comprehensive default
func (h *ChatWebSocketHandler) getSystemPromptOrDefault(ctx context.Context, userID, companyID string) string {
	// Try to get user's custom system prompt
