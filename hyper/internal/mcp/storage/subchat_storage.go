...
// buildSubagentSystemPrompt prepends the BaseSubagentPrompt to a subagent's specific prompt
func buildSubagentSystemPrompt(specificPrompt string) string {
	return BaseSubagentPrompt + "\n\n## YOUR SPECIALIZATION\n\n" + specificPrompt
}
	...
	systemSubagents := []Subagent{
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are a Go backend development specialist. Focus on writing clean, efficient Go code following best practices."),
			Category:     "backend",
			Tools:        []string{"*"},
		},
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are a Go MCP development specialist. Focus on implementing MCP tools and protocols in Go."),
			Category:     "backend",
			Tools:        []string{"*"},
		},
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are a Backend Services Specialist focusing on Go 1.25 microservices, REST APIs, and service architecture within the Hyperion AI Platform."),
			Category:     "backend",
		...
		},
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are an Event Systems Specialist focusing on NATS JetStream, MCP protocols, event-driven architecture, and service orchestration."),
			Category:     "backend",
			Tools:        []string{"hyper", "@modelcontextprotocol/server-filesystem", "@modelcontextprotocol/server-github", "@modelcontextprotocol/server-fetch", "mcp-server-mongodb"},
		},
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are a Data Platform Specialist focusing on MongoDB, data modeling, query performance, migration strategies, and vector operations."),
			Category:     "data",
			Tools:        []string{"hyper", "@modelcontextprotocol/server-filesystem", "@modelcontextprotocol/server-github", "@modelcontextprotocol/server-fetch", "mcp-server-mongodb"},
		},
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are a frontend UI development specialist. Focus on React, TypeScript, and modern UI frameworks."),
			Category:     "frontend",
			Tools:        []string{"*"},
		},
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are a UI testing specialist. Focus on testing design, layout, and stability issues in web projects."),
			Category:     "frontend",
			Tools:        []string{"*"},
		},
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are a Frontend Experience Specialist focusing on React 18, TypeScript, atomic design systems, user experience, accessibility, and component architecture."),
			Category:     "frontend",
		...
		},
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are an AI Integration Specialist focusing on Claude/GPT APIs, AI3 framework, model coordination, intelligent task orchestration, and AI-driven user experiences."),
			Category:     "ai",
			Tools:        []string{"hyper", "@modelcontextprotocol/server-filesystem", "@modelcontextprotocol/server-github", "playwright-mcp", "@modelcontextprotocol/server-fetch"},
		},
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are a Real-time Systems Specialist focusing on WebSockets, real-time protocols, streaming data delivery, live connections, and real-time synchronization."),
		...
		},
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are an SRE specialist. Focus on deployment, monitoring, reliability, and operational excellence."),
			Category:     "infrastructure",
			Tools:        []string{"*"},
		},
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are a Kubernetes deployment expert. Focus on creating, modifying, troubleshooting, and optimizing Kubernetes deployments, including manifests, rollouts, scaling strategies, and resource management."),
			Category:     "infrastructure",
			Tools:        []string{"*"},
		},
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are an Infrastructure Automation Specialist focusing on Google Cloud Platform, Kubernetes, GKE deployment automation, GitHub Actions CI/CD, and infrastructure orchestration."),
			Category:     "infrastructure",
		...
		},
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are a Security & Auth Specialist focusing on security architecture, JWT authentication, identity management, access control, security policies, and threat protection."),
			Category:     "security",
			Tools:        []string{"hyper", "mcp-server-kubernetes", "@modelcontextprotocol/server-github", "@modelcontextprotocol/server-filesystem", "@modelcontextprotocol/server-fetch"},
		},
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are an Observability Specialist focusing on monitoring, observability, metrics collection, distributed tracing, performance analysis, and operational insights."),
			Category:     "infrastructure",
			Tools:        []string{"hyper", "mcp-server-kubernetes", "@modelcontextprotocol/server-github", "@modelcontextprotocol/server-filesystem", "@modelcontextprotocol/server-fetch"},
		},
		{
			...
			SystemPrompt: buildSubagentSystemPrompt("You are an End-to-End Testing Coordinator focusing on cross-squad testing orchestration, end-to-end automation, user journey validation, integration testing, and quality assurance coordination."),
			Category:     "testing",
