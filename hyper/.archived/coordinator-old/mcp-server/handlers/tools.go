...
func (h *ToolHandler) handleListAgentTasks(ctx context.Context, args map[string]interface{}) (*mcp.CallToolResult, map[string]interface{}, error) {
	...
	for i, task := range paginatedTasks {
		// CRITICAL: Verify task is not nil and ID is a proper string
		if task == nil {
			continue // Skip nil tasks
		}

		taskMap := make(map[string]interface{})
		// Explicit string assertion to ensure proper serialization
		taskID := task.ID
		if taskID == "" {
			taskID = "EMPTY_ID"  // Debug: mark empty IDs
		}
		taskMap["id"] = taskID
		...
		for j, todo := range task.Todos {
			...
			// Explicit string assertion for TODO ID
			todoID := todo.ID
			if todoID == "" {
				todoID = "EMPTY_TODO_ID"  // Debug: mark empty TODO IDs
			}
			todoMap["id"] = todoID
