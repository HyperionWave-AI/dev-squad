...
func (h *ChatWebSocketHandler) HandleChatWebSocket(c *gin.Context) {
	...
	if err != nil {
		// Session not found - create a new one automatically with the provided ID
		h.logger.Info("Session not found, creating new session automatically",
			zap.String("sessionId", sessionID.Hex()),
			zap.String("userId", userID),
			zap.String("companyId", companyID))

		// Create new session with the same ID (seamless recovery)
		session, err = h.chatService.CreateSessionWithID(c.Request.Context(), sessionID, userID, companyID, "New Chat")
		if err != nil {
			h.logger.Error("Failed to auto-create session", zap.Error(err))
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to create session: " + err.Error()})
			return
		}

		h.logger.Info("Session auto-created successfully",
			zap.String("sessionId", session.ID.Hex()),
			zap.String("userId", userID))
	} else {
		// Session exists - verify it belongs to the user
...
func (h *ChatWebSocketHandler) streamAIResponse(ctx context.Context, conn *websocket.Conn, sessionID primitive.ObjectID, userMessage, companyID string) {
	...
	h.logger.Debug("Retrieved conversation history",
		zap.String("sessionId", sessionID.Hex()),
		zap.Int("messageCount", conversationMessageCount))

	// Step 2: Convert MongoDB messages to LangChain format
	langchainMessages := aiservice.ConvertToLangChainMessages(messages)

	// Step 2.5: Get system prompt (subagent > user custom > default) and prepend it
	session, _ := h.chatService.GetSession(ctx, sessionID, companyID)
	userID := ""
	if session != nil {
		userID = session.UserID
	}
	systemPrompt := h.getSystemPromptOrDefault(ctx, session, userID, companyID)

	// Prepend system message at index 0 (before conversation history)
	langchainMessages = append([]aiservice.Message{{
		Role:    "system",
		Content: systemPrompt,
	}}, langchainMessages...)

	h.logger.Debug("System prompt injected",
		zap.String("sessionId", sessionID.Hex()),
		zap.Int("promptLength", len(systemPrompt)),
		zap.Int("totalMessages", len(langchainMessages)))

	// Step 3: Stream AI response via ai-service with tool support
	...
	// Step 4: Stream mixed content (tokens and tool events) to WebSocket client
	fullResponse := ""
	tokenCount := 0
	toolCallCount := 0
	stopReason := "natural_completion" // Default: AI decided to stop
	...
	for event := range aiStream {
		...
		case <-ctx.Done():
			...
			h.logger.Info("Context cancelled during streaming",
				...
				zap.Int("conversationMessages", conversationMessageCount),
		...
		default:
			...
			case aiservice.StreamEventError:
				...
				// Check if it's max tool calls error
				if event.Error == fmt.Sprintf("maximum tool calls (%d) exceeded", maxToolCalls) {
				...
				duration := time.Since(startTime)
				h.logger.Error("AI service error during streaming",
					zap.String("sessionId", sessionID.Hex()),
					zap.String("stopReason", stopReason),
	...
	// Calculate total duration and log detailed completion stats
	duration := time.Since(startTime)
	h.logger.Info("AI response streamed successfully",
		...
		zap.Duration("duration", duration),
		zap.Int("maxToolCallsLimit", maxToolCalls))
}
...
// getSystemPromptOrDefault retrieves system prompt with priority: subagent > user custom > default
// When a subagent is assigned to a session, its system prompt takes precedence over the user's custom prompt
func (h *ChatWebSocketHandler) getSystemPromptOrDefault(ctx context.Context, session *models.ChatSession, userID, companyID string) string {
	// Priority 1: Check if session has an assigned subagent
	if session != nil && session.SubagentID != nil && h.aiSettingsService != nil {
		subagent, err := h.aiSettingsService.GetSubagent(ctx, *session.SubagentID, companyID)
		if err == nil && subagent != nil && subagent.SystemPrompt != "" {
			h.logger.Debug("Using subagent system prompt",
				zap.String("sessionId", session.ID.Hex()),
				zap.String("subagentId", session.SubagentID.Hex()),
				zap.String("subagentName", subagent.Name),
				zap.Int("promptLength", len(subagent.SystemPrompt)))
			return subagent.SystemPrompt
		}
		if err != nil {
			h.logger.Warn("Failed to retrieve subagent system prompt, falling back to user prompt",
				zap.Error(err),
				zap.String("subagentId", session.SubagentID.Hex()))
		}
	}

	// Priority 2: Try to get user's custom system prompt
	if h.aiSettingsService != nil && userID != "" {
		customPrompt, err := h.aiSettingsService.GetSystemPrompt(ctx, userID, companyID)
		if err == nil && customPrompt != "" {
			h.logger.Debug("Using custom system prompt",
				zap.String("userId", userID),
				zap.Int("promptLength", len(customPrompt)))
			return customPrompt
		}
	}

	// Return comprehensive default that encourages thorough tool usage
	defaultPrompt := `You are a helpful AI assistant with access to various tools for reading files, managing tasks, and querying knowledge bases.

**When responding to user requests:**

1. **Be Thorough**: Don't stop after reading just one or two files. If asked to "analyze the project", read multiple files to understand the architecture, patterns, and structure.

2. **Use Tools Extensively**: You have access to powerful tools - use them! Read files, list directories, search code, and query knowledge bases to gather complete information.

3. **Plan Before Acting**: When given a complex task like "analyze the project", think about what files would be most informative (README, main entry points, key modules) and read them systematically.

4. **Provide Context**: Explain what you're doing as you work through a task. Tell the user which files you're examining and what you're learning from them.

5. **Don't Assume**: If you need information, use tools to get it rather than making assumptions or saying you don't know.
...
- Knowledge base queries for finding patterns and solutions

**Remember:** Your goal is to be comprehensive and helpful. Use tools liberally to gather all the information needed to provide accurate, complete answers.`

	h.logger.Debug("Using default system prompt", zap.Int("promptLength", len(defaultPrompt)))
