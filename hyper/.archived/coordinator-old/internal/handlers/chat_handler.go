...
func NewChatHandler(chatService *services.ChatService, aiSettingsService *services.AISettingsService, logger *zap.Logger) *ChatHandler {
	return &ChatHandler{
...
// AssignSubagent assigns a subagent to a chat session
// PUT /api/v1/chat/sessions/:id/subagent
func (h *ChatHandler) AssignSubagent(c *gin.Context) {
	_, companyID, err := h.extractUserContext(c)
	if err != nil {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "Unauthorized: " + err.Error()})
		return
	...
	var req struct {
		SubagentID string `json:"subagentId" binding:"required"`
	}
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid request: " + err.Error()})
		return
	...
	// Parse subagent ID
	subagentID, err := primitive.ObjectIDFromHex(req.SubagentID)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid subagent ID"})
		return
	}

	// Look up subagent to get its name
	subagent, err := h.aiSettingsService.GetSubagent(c.Request.Context(), subagentID, companyID)
	if err != nil {
		h.logger.Error("Failed to get subagent", zap.Error(err))
		c.JSON(http.StatusNotFound, gin.H{"error": "Subagent not found"})
		return
	}

	// Assign subagent to session
	updatedSession, err := h.chatService.AssignSubagent(
		c.Request.Context(),
		sessionID,
		subagentID,
		subagent.Name,
	...
	if err != nil {
		h.logger.Error("Failed to assign subagent", zap.Error(err))
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *ChatHandler) RegisterChatRoutes(r *gin.RouterGroup) {
	...
	r.PUT("/sessions/:id/subagent", h.AssignSubagent)
}
