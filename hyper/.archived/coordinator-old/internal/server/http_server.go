...
func StartHTTPServer(
	...
	// Get Claude agents directory from environment or default to project root
	claudeAgentsDir := os.Getenv("CLAUDE_AGENTS_DIR")
	if claudeAgentsDir == "" {
		claudeAgentsDir = "/Users/maxmednikov/MaxSpace/dev-squad/.claude/agents"
	}
	aiSettingsService, err := services.NewAISettingsService(mongoDatabase, claudeAgentsDir, logger)
	if err != nil {
		logger.Error("Failed to initialize AI settings service", zap.Error(err))
		return err
	}
	logger.Info("AI settings service initialized", zap.String("claudeAgentsDir", claudeAgentsDir))

	// Initialize subchat service
	subchatService := services.NewSubchatService(mongoDatabase)
	logger.Info("Subchat service initialized successfully")
	...
	chatHandler := handlers.NewChatHandler(chatService, aiSettingsService, logger)
	...
	// Create subchat handler
	subchatHandler := handlers.NewSubchatHandler(subchatService)
	...
	handlers.RegisterSubchatRoutes(apiV1, subchatHandler)
	logger.Info("Subchat API routes registered",
		zap.String("createSubchatPath", "/api/v1/subchats"),
		zap.String("getSubchatsPath", "/api/v1/chats/:parentId/subchats"))
		...
		r.Any("/ui/*proxyPath", func(c *gin.Context) {
			proxy := &httputil.ReverseProxy{
				Director: func(req *http.Request) {
					...
					req.URL.Path = c.Request.URL.Path // Keep /ui prefix for Vite
		...
		r.GET("/ui", func(c *gin.Context) {
			proxy := &httputil.ReverseProxy{
				Director: func(req *http.Request) {
					req.URL.Scheme = "http"
					req.URL.Host = "localhost:5173"
					req.URL.Path = "/ui/" // Vite serves at /ui/ base path
					req.Host = "localhost:5173"
				},
			}
			proxy.ServeHTTP(c.Writer, c.Request)
