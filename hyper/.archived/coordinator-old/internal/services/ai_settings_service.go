...
func NewAISettingsService(db *mongo.Database, claudeAgentsDir string, logger *zap.Logger) (*AISettingsService, error) {
	service := &AISettingsService{
		systemPromptsCollection: db.Collection("system_prompts"),
		subagentsCollection:     db.Collection("subagents"),
		claudeAgentsDir:         claudeAgentsDir,
		logger:                  logger,
	}
...
func (s *AISettingsService) ListClaudeAgents(ctx context.Context) ([]models.ClaudeAgent, error) {
	...
	// Read .claude/agents directory using configured path
	pattern := filepath.Join(s.claudeAgentsDir, "*.md")
	...
	if len(files) == 0 {
		s.logger.Debug("No Claude agent files found", zap.String("directory", s.claudeAgentsDir))
		return agents, nil
	}
...
func (s *AISettingsService) ImportClaudeAgents(ctx context.Context, userID, companyID string, agentNames []string) (int, []string, error) {
	...
	for _, agentName := range agentNames {
		...
		// Read agent file
		filePath := filepath.Join(s.claudeAgentsDir, fmt.Sprintf("%s.md", agentName))
		content, err := os.ReadFile(filePath)
		if err != nil {
