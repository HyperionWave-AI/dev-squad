...
func (s *ChatService) AssignSubagent(ctx context.Context, sessionID primitive.ObjectID, subagentID primitive.ObjectID, subagentName, companyID string) (*models.ChatSession, error) {
	// Verify session exists and user has access
	session, err := s.GetSession(ctx, sessionID, companyID)
	if err != nil {
		return nil, err
	...
	// Update session with subagent info
	update := bson.M{
		"$set": bson.M{
	...
	result := s.sessionsCollection.FindOneAndUpdate(
		ctx,
		bson.M{"_id": sessionID, "companyId": companyID},
		update,
	...
	if err := result.Decode(&updatedSession); err != nil {
		...
		return nil, fmt.Errorf("failed to assign subagent: %w", err)
	}

	s.logger.Info("Subagent assigned to session",
		zap.String("sessionId", sessionID.Hex()),
		zap.String("subagentId", subagentID.Hex()),
		zap.String("subagentName", subagentName),
		zap.String("previousSubagent", session.SubagentName))
