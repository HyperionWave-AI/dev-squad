...
// NewSubagentSystemPrompt is the improved system prompt for subagents
// This enforces strict tool usage based on TOOL_USAGE_GUIDANCE.md
const NewSubagentSystemPrompt = `You are a specialized AI subagent with access to powerful tools for code analysis, file operations, and task execution. Your primary responsibility is to use the CORRECT tool for each specific purpose. Using the wrong tool wastes time and resources.
...
func main() {
	...
	db := client.Database("dev-squad")
	collection := db.Collection("subagents")

	// Update all subagents with the new system prompt
	ctx, cancel = context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	result, err := collection.UpdateMany(ctx, bson.M{}, bson.M{
		"$set": bson.M{
			"systemPrompt": NewSubagentSystemPrompt,
			"updatedAt":    time.Now().UTC(),
		},
	})

	if err != nil {
		log.Fatalf("Failed to update subagents: %v", err)
	}

	fmt.Printf("✅ Successfully updated %d subagents with new system prompt\n", result.ModifiedCount)
	fmt.Printf("📊 Matched: %d, Modified: %d, Upserted: %d\n", result.MatchedCount, result.ModifiedCount, result.UpsertedID)
	fmt.Println("\n📝 New system prompt enforces strict tool usage:")
	fmt.Println("   - code_index_search for semantic code discovery (not bash find)")
	fmt.Println("   - read_file for file reading (not bash cat)")
	fmt.Println("   - write_file for file writing (not bash echo)")
	fmt.Println("   - bash ONLY for actual system operations (build, test, etc.)")
	fmt.Println("   - Proper tool selection matrix for all operations")
	fmt.Println("   - Complete code requirement (no placeholders)")
	fmt.Println("   - Compilation verification mandatory")
}
